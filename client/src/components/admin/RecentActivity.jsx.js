...
const RecentActivity = ({ activities = [] }) => {
  const mockActivities = [
    { id: 1, type: 'test_completed', user: 'John Doe', details: 'Monthly Dictation Challenge', time: '2 min ago', status: 'success' },
    { id: 2, type: 'test_created', user: 'Admin', details: 'Advanced Transcription Test', time: '5 min ago', status: 'info' },
    { id: 3, type: 'score_improved', user: 'Sarah Wilson', details: '82% → 89%', time: '12 min ago', status: 'success' },
    { id: 4, type: 'new_student', user: 'New User', details: 'Registered', time: '1 hr ago', status: 'info' },
  ];
...
const getIcon = (type, testType) => {
    if (type === 'test_activity') {
      switch (testType) {
        case 'dictation': return <FileText className="w-5 h-5 text-indigo-400" />;
        case 'transcription': return <Activity className="w-5 h-5 text-purple-400" />;
        default: return <TrendingUp className="w-5 h-5 text-emerald-400" />;
      }
    }
    switch (type) {
      case 'test_completed': return <CheckCircle className="w-5 h-5 text-emerald-400" />;
      case 'test_created': return <FileText className="w-5 h-5 text-indigo-400" />;
      case 'score_improved': return <TrendingUp className="w-5 h-5 text-emerald-400" />;
