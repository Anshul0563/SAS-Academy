...
function Result() {
    ...
    const run = async () => {
      ...
      try {
        ...
        const serverMessage =
          ...
          submitError.message;
        setError(
          `Result was calculated locally, but it could not be saved right now. ${serverMessage ? `Reason: ${serverMessage}` : ""}`,
        );
  ...
  const scoreCards = [
    ...
    },
    {
      ...
      tone: "text-amber-200",
      detail: `${data.correctWords || 0}/${data.totalWords || 0} words`,
    },
    {
      label: "Time",
  ...
  const breakdown = [
    ...
    {
      label: "Backspace",
      value: data.backspaces || Number(localStorage.getItem("backspace")) || 0,
    },
  ];
  ...
  const getWordStyle = (type) => {
    const styles = {
      ...
      omission:
        "rounded bg-sky-500/15 px-1 font-medium text-sky-200 line-through decoration-sky-200/80",
      addition:
        "rounded bg-amber-400/15 px-1 font-medium text-amber-100 underline decoration-amber-200/80 underline-offset-4",
      spelling:
        "rounded bg-red-500/15 px-1 font-medium text-red-200 underline decoration-red-200/80 decoration-wavy underline-offset-4",
    };
  ...
  return (
    <div className="min-h-dvh bg-slate-950 text-white">
      <div
        ref={resultRef}
        className="mx-auto flex max-w-6xl flex-col gap-5 px-3 py-5 sm:px-6 lg:px-8"
      >
        <header className="flex flex-col gap-4 border-b border-white/10 pb-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
              Net WPM is calculated from correct five-character strokes, so
              extra or misspelled text does not inflate the final speed.
            </p>
            {error && (
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {scoreCards.map((item) => (
            ...
            >
              ...
              <p className={`mt-3 text-4xl font-bold ${item.tone}`}>
                {item.value}
              </p>
        ...
        <section className="grid gap-4 lg:grid-cols-[1fr_1.35fr]">
          <div className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
              {breakdown.map((item) => (
                <div
                  key={item.label}
                  className="rounded-md bg-slate-900/80 px-3 py-3"
                >
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
            ...
            <div className="mt-4 max-h-[60vh] overflow-y-auto rounded-md bg-slate-900/80 p-4">
              {data.comparison?.length ? (
                <div className="flex flex-wrap gap-x-2 gap-y-3 text-base leading-8 text-slate-200">
