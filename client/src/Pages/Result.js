  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        setError(
          `Result was calculated locally, but it could not be saved right now. ${
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
            ...
            <div className="mt-4 max-h-[360px] overflow-y-auto rounded-md bg-slate-900/80 p-4">
              {data.comparison?.length ? (
                <div className="flex flex-wrap items-start gap-x-2 gap-y-3 text-base leading-9 text-slate-200 break-all">
                  {data.comparison.map((item, index) => (
                    ...
                    >
                      <span className={getWordStyle(item.type)}>
