...
const getUnits = (originalText, typedText, options) => {
  ...
  const characterMode = Boolean(typed && !typed.includes(" "));
...
const buildLinearComparison = (originalUnits, typedUnits) => {
  ...
  while (i < originalUnits.length || j < typedUnits.length) {
    ...
    if (expected && typed && expected === typed) {
      comparison.push({ expected, typed, word: typed, type: "correct" });
      i += 1;
      j += 1;
    } else if (typed && originalUnits[i] === typedUnits[j + 1]) {
      comparison.push({ expected: "", typed, word: typed, type: "addition" });
      j += 1;
    } else if (expected && originalUnits[i + 1] === typedUnits[j]) {
      comparison.push({ expected, typed: "", word: expected, type: "omission" });
      i += 1;
    } else if (expected && typed) {
      comparison.push({ expected, typed, word: typed, type: "spelling" });
      i += 1;
      j += 1;
    } else if (typed) {
      comparison.push({ expected: "", typed, word: typed, type: "addition" });
...
const buildWordComparison = (originalWords, typedWords) => {
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && dp[i][j] === dp[i - 1][j - 1] + (originalWords[i - 1] === typedWords[j - 1] ? 0 : 1)) {
      ...
      comparison.unshift({ expected: "", typed: typedWords[j - 1], word: typedWords[j - 1], type: "addition" });
      j -= 1;
    } else {
      comparison.unshift({ expected: originalWords[i - 1], typed: "", word: originalWords[i - 1], type: "omission" });
      i -= 1;
    }
  }
...
const getComparisonWord = (item) => (item.type === "omission" ? item.expected : item.word || item.typed);
...
const groupComparisonForDisplay = (comparison = []) => {
  ...
  comparison.forEach((item) => {
    ...
    groups.push({ base: item, additions: leadingAdditions });
    leadingAdditions = [];
  });

  if (leadingAdditions.length) groups.push({ base: null, additions: leadingAdditions });
  return groups;
};
...
const calculateLocalResult = ({ originalText, typedText, timeUsedSeconds, settings }) => {
  ...
  const { mode, originalUnits, typedUnits } = getUnits(originalText, typedText, options);
  const comparison = mode === "characters"
  ...
  const correctCharacters = comparison
    ...
    .reduce((total, item) => total + String(item.typed || item.word || "").length, 0);
...
function Result() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const localResult = {
        ...calculateLocalResult({ originalText, typedText, timeUsedSeconds, settings }),
        backspaces,
        keystrokes,
        saved: false,
      };
      ...
      try {
        ...
        if (!cancelled) {
          setData({ ...localResult, _id: res.data?._id, saved: true });
          setSaveError("");
        }
      } catch (saveFailure) {
        const reason = saveFailure.response?.data?.detail || saveFailure.response?.data?.message || saveFailure.message;
        console.error("Compact result save error:", saveFailure);
        if (!cancelled) {
          setData(localResult);
          setSaveError(`Result calculated, but DB save failed. Reason: ${reason}`);
        }
      }
    };
  ...
  const scoreCards = [
    { label: "Net WPM", value: formatNumber(data.netWPM), detail: "Correct strokes", icon: Gauge, tone: "text-emerald-300" },
    { label: "Accuracy", value: `${formatNumber(data.accuracy)}%`, detail: `${data.correctWords || 0}/${data.totalWords || 0} correct`, icon: Target, tone: "text-amber-200" },
    { label: "Gross WPM", value: formatNumber(data.grossWPM), detail: "All typed strokes", icon: Keyboard, tone: "text-sky-300" },
    { label: "Time", value: formatTime(data.timeTaken), detail: `${data.timeTaken || 0} seconds`, icon: Timer, tone: "text-violet-200" },
  ];

  const detailStats = [
    { label: "Correct", value: data.correctWords || 0, icon: CheckCircle2, tone: "text-emerald-300" },
    { label: "Errors", value: errors, icon: XCircle, tone: "text-red-300" },
    { label: "Omissions", value: data.omissions || 0, icon: MinusCircle, tone: "text-sky-300" },
    { label: "Additions", value: data.additions || 0, icon: PlusCircle, tone: "text-amber-200" },
  ];

  const getWordStyle = (type) => {
    const styles = {
      correct: "text-emerald-300",
      omission: "rounded bg-sky-500/15 px-1 text-sky-200 line-through decoration-sky-200/80",
      addition: "rounded bg-amber-400/15 px-1 text-amber-100 underline decoration-amber-200/80 underline-offset-4",
      spelling: "rounded bg-red-500/15 px-1 text-red-200 underline decoration-red-200/80 decoration-wavy underline-offset-4",
    };
  ...
  return (
    <div className="min-h-dvh bg-[#020617] text-white">
      <div ref={resultRef} className="mx-auto flex max-w-7xl flex-col gap-5 px-3 py-5 sm:px-6 lg:px-8">
        <header className="rounded-lg border border-white/10 bg-white/[0.04] p-5 shadow-xl sm:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">Performance scorecard</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
                Your score is calculated locally for a fast result page, then a compact score is saved to the database for the leaderboard.
              </p>
              <div className={`mt-4 inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm ${saveTone}`}>
                {data.saved ? <CheckCircle2 size={16} /> : <AlertTriangle size={16} />}
                {data.saved ? "Result saved to database" : saveError || "Result not saved yet"}
              </div>
            </div>

            <div className="flex flex-wrap gap-2">
              <button onClick={() => navigate("/dashboard")} className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-3 text-sm font-semibold transition hover:bg-indigo-500">
                <BarChart3 size={16} /> Dashboard
              </button>
              <button onClick={() => navigate(`/typing/${localStorage.getItem("testId")}`)} className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 py-3 text-sm font-semibold transition hover:bg-emerald-500">
                <RefreshCcw size={16} /> Retry
              </button>
              <button onClick={downloadResult} className="inline-flex items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-semibold transition hover:bg-white/10">
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {scoreCards.map((item) => {
            ...
            return (
              <div key={item.label} className="rounded-lg border border-white/10 bg-white/[0.04] p-4 shadow-xl">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{item.label}</p>
                  <Icon size={18} className={item.tone} />
                </div>
                <p className={`mt-4 text-4xl font-bold ${item.tone}`}>{item.value}</p>
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.04] p-4 shadow-xl">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              {detailStats.map((item) => {
                const Icon = item.icon;
                return (
                  <div key={item.label} className="rounded-md border border-white/10 bg-slate-950/70 p-3">
            ...
            <div className="mt-4 rounded-md border border-white/10 bg-slate-950/70 p-3 text-sm leading-6 text-slate-300">
              ...
              <p>Comparison mode: <span className="font-semibold capitalize text-white">{data.comparisonMode || "words"}</span></p>
            </div>
          </div>
                ...
                <p className="text-base leading-9 text-slate-200">
                  {comparisonGroups.map((group, groupIndex) => (
                    <span key={groupIndex} className="mr-1.5 inline-flex items-baseline gap-1 whitespace-nowrap align-baseline">
                      {group.base && (
                        <span className={getWordStyle(group.base.type)} title={group.base.expected && group.base.typed ? `${group.base.expected} -> ${group.base.typed}` : undefined}>
                          {getComparisonWord(group.base)}
                        </span>
                      )}
                      {group.additions.map((addition, additionIndex) => (
                        <span key={`${groupIndex}-${additionIndex}`} className={getWordStyle(addition.type)} title="Extra typed item">
